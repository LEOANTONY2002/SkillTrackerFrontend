...
import {
  useAddPublisher,
  useDeletePublisher,
} from "../../graphql/mutation/usePublisher";
import {
  useGetAllCertificates,
  useGetAllPublishers,
} from "../../graphql/query/useGetAllCertificates";
import {
  useGetSearchCertificates,
  useGetSearchCertificatesByPublisher,
...
import iCertW from "../../assets/iCertW.png";
...
function Certificate() {
  ...
  const { loading: gettingCertificates, certificates = [] } =
    useGetAllCertificates();
  const { searchCertificates, loading: searchingCertificate } =
    useGetSearchCertificates();
  const {
    searchCertificatesByPublisher,
    loading: searchingCertificateByPublisher,
  } = useGetSearchCertificatesByPublisher();
  ...
  const searchCertificateByPublisher = async () => {
    ...
    if (data?.searchCertificateByPublisher?.length === 0) {
      ...
      setSearch({
        ...search,
        certificates: data?.searchCertificateByPublisher,
      });
    }
  };
  ...
  const ascendingCategory = async () => {
    ...
    let ascCertificates = certificates.sort((a, b) =>
  ...
  const descendingCategory = async () => {
    let certificates = [...displayCertificates];
    let descCertificates = certificates.sort((a, b) =>
      a?.name < b?.name ? 1 : b?.name < a?.name ? -1 : 0
    );
  ...
  return (
    <>
      <div className="category">
        {gettingPublishers || deletingPublisher ? (
          <div
            style={{ height: "200px", display: "grid", placeContent: "center" }}
          >
              ...
              <div className="add">
                <div className="c-add">
                  ...
                  <div className="ca-inp">
                    <input
                      type="text"
                      onKeyDown={(e) => e.key === "Enter" && upsertPublisher()}
        ...
        <div style={{ marginTop: "50px" }} className="search">
          ...
          <div className="sh-filter">
            ...
            <select
              onChange={(e) => setSelectedOptionPublisher(e.target.value)}
            >
              <option selected={selectedOptionPublisher === null} disabled>
                Publisher
              </option>
              {publishers?.map((p) => (
        ...
        {gettingCertificates ||
        searchingCertificate ||
        searchingCertificateByPublisher ? (
          <div
            style={{ height: "50vh", display: "grid", placeContent: "center" }}
          >
            <img style={{ width: "40px" }} src={loader} alt="" />
          </div>
            ...
            {displayCertificates?.map((ds) => (
              <div className="certificate">
                ...
                <div className="ct-img">
                  <img
                    onClick={() => setZoom({ open: true, cert: ds })}
                    src={ds?.photo}
                    alt=""
                  />
                </div>
        ...
        <div className="c-edit">
          <div>
            <div className="ce-head">
              ...
              <p>Edit Publisher</p>
              <img
                onClick={() => setPublisher({ open: false, id: "", name: "" })}
                src={iCloseR}
                alt=""
              />
            </div>
