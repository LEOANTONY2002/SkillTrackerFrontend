...
function Employee() {
  ...
  const {loading: gettingEmployees, employees: employees=[], error: errorEmployees} = useGetAllEmployees()
  const {loading: searchingEmployees, searchEmployees} = useGetSearchEmployees()
  const {loading: searchingEmployeesBySkill, searchEmployeesBySkill} = useGetSearchEmployeesBySkill()
  ...
  const searchEmployeeBySkill = async () => {
    ...
    if (data?.searchEmployeeBySkill?.length === 0) {
      ...
      setSearch({...search, word: "", employees: data?.searchEmployeeBySkill})
      setLoading(false)
    }
  }
    ...
    <div className="employee">
      ...
      <div className="search">
          <div className="sh-body">
            <input type="text" onKeyDown={e => e.key === 'Enter' && searchEmployee()} placeholder="Search Employees..." value={search.word} onChange={(e) => setSearch({...search, word: e.target.value})} required />
            <img
        ...
        {(searchingEmployees  || searchingEmployeesBySkill || gettingEmployees) ? 
          ...
          <>
                    ...
                    <div className="emp" key={e?.id}>
                      <div className="e-head">
                        <img
                          style={
                            e?.photo !== ""
                              ? { padding: "0px", width: "60px", height: "60px" }
                              : { padding: "10px" }
                          }
                          src={
                            e?.photo !== ""
            ...
            <h6 style={{color: "red", marginTop: "30px", fontSize: "smaller", wordBreak: "keep-all", textAlign: "center"}}>{!gettingEmployees && 'No records found!'}</h6>}

            { displayEmployees === employees && <p style={{marginTop: "50px", color: "red", fontSize: "small"}}>Other Employees</p> }
